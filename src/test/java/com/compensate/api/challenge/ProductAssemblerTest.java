...
class ProductAssemblerTest {
  ...
  public void testToModelAddsParentAndChildrenLinksToProductResource() {
    ...
    assertThat(actualProduct.getLink("parent").get().toString())
        .isEqualTo(
            Link.of("/api/v1/products/" + productEntity.getParent().getId(), "parent").toString());
    assertThat(actualProduct.getLink("root").get().toString())
        .isEqualTo(
            Link.of("/api/v1/products/" + productEntity.getParent().getId(), "root").toString());
    assertThat(actualProduct.getLink("subProducts").get().toString())
        .isEqualTo(
            Link.of("/api/v1/products/" + productEntity.getId() + "/subproducts?page=0&size=10",
                "subProducts").toString());
  }
}